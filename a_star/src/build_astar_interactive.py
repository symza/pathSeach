...
"""
A*交互式演示程序打包脚本
将 astar_interactive.py 打包为单个可执行文件
"""
import os
import sys
import subprocess
...
A_STAR_DIR = os.path.dirname(SCRIPT_DIR)  # a_star文件夹
...
def clean_old_files():
    ...
    # 清理src和a_star文件夹中的build和dist目录
    dirs_to_remove = ['build', 'dist']
    for base_dir in [SCRIPT_DIR, A_STAR_DIR]:
    ...
    # 删除spec文件（在src和a_star文件夹中）
    for base_dir in [SCRIPT_DIR, A_STAR_DIR]:
        spec_file = os.path.join(base_dir, 'AStarInteractive.spec')
...
def build_exe(debug=False):
    ...
    # 源文件在src文件夹中
    source_file = os.path.join(SCRIPT_DIR, "astar_interactive.py")
    ...
    try:
        ...
        # 确定输出文件名
        exe_name = "AStarInteractive_Debug" if debug else "AStarInteractive"
        ...
        # --name: 指定输出文件名
        # --clean: 清理临时文件
        # --distpath: 指定dist目录位置（a_star文件夹）
        # --workpath: 指定build目录位置（a_star文件夹）
        ...
        cmd = [
            ...
            "--clean",
            "--distpath", A_STAR_DIR,
            "--workpath", os.path.join(A_STAR_DIR, "build"),
        ...
        cmd.append("astar_interactive.py")
        
        subprocess.check_call(cmd)
        
        # 移动exe文件到a_star文件夹（如果不在的话）
        exe_src = os.path.join(A_STAR_DIR, f"{exe_name}.exe")
        exe_dst = os.path.join(A_STAR_DIR, f"{exe_name}.exe")
