...
"""
路径搜索算法演示程序打包脚本
将 pathfinding_maze_app.py 打包为单个可执行文件
"""
import os
import sys
import subprocess
...
# 获取脚本所在目录和pathfinding文件夹路径
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))  # src文件夹
PATHFINDING_DIR = os.path.dirname(SCRIPT_DIR)  # pathfinding文件夹
...
def clean_old_files():
    ...
    # 清理src和pathfinding文件夹中的build和dist目录
    dirs_to_remove = ['build', 'dist']
    for base_dir in [SCRIPT_DIR, PATHFINDING_DIR]:
    ...
    # 删除spec文件（在src和pathfinding文件夹中）
    spec_names = ['PathfindingMazeApp.spec', 'PathfindingMazeDemo.spec']
    for base_dir in [SCRIPT_DIR, PATHFINDING_DIR]:
        for spec_name in spec_names:
            spec_file = os.path.join(base_dir, spec_name)
...
def build_exe(debug=False):
    ...
    # 源文件在src文件夹中
    source_file = os.path.join(SCRIPT_DIR, "pathfinding_maze_app.py")
    ...
    try:
        ...
        # 确定输出文件名
        exe_name = "PathfindingMazeApp_Debug" if debug else "PathfindingMazeApp"
        ...
        # --name: 指定输出文件名
        # --clean: 清理临时文件
        # --distpath: 指定dist目录位置（pathfinding文件夹）
        # --workpath: 指定build目录位置（pathfinding文件夹）
        ...
        cmd = [
            ...
            "--name", exe_name,
            "--clean",
            "--distpath", PATHFINDING_DIR,
            "--workpath", os.path.join(PATHFINDING_DIR, "build"),
        ...
        cmd.append("pathfinding_maze_app.py")
        
        subprocess.check_call(cmd)
        
        # 移动exe文件到pathfinding文件夹（如果不在的话）
        exe_src = os.path.join(PATHFINDING_DIR, f"{exe_name}.exe")
        exe_dst = os.path.join(PATHFINDING_DIR, f"{exe_name}.exe")
